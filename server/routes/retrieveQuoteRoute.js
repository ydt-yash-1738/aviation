...
router.get('/search', async (req, res) => {
  try {
    ...
    // Search by completedQuoteData.insuredFirstName
    if (insuredFirstName && insuredFirstName.trim() !== '') {
      const regex = new RegExp(`^${escapeRegex(insuredFirstName.trim())}`, 'i');
      query['completedQuoteData.insuredFirstName'] = regex;
    }

    // Search by completedQuoteData.effectiveDate (range)
    if (effectiveDate) {
      const inputDate = new Date(effectiveDate);
      const startOfDay = new Date(inputDate);
      const endOfDay = new Date(inputDate);
      ...
      query['completedQuoteData.effectiveDate'] = {
        $gte: startOfDay,
        $lte: endOfDay
      };
    }

    // Search by policyNumber (top-level field)
    if (policyNumber && policyNumber.trim() !== '') {
      query.policyNumber = policyNumber.trim();
    }

    // Search by quoteRef (top-level field)
    if (quoteRef && quoteRef.trim() !== '') {
      query.quoteRef = quoteRef.trim();
    }

    const results = await Quote.find(query).sort({ createdAt: -1 });
    res.status(200).json(results);
  } catch (error) {
